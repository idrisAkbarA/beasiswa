    ...
    <v-navigation-drawer style="z-index:11" :src="'/images/drawer-bg.jpg'" v-model="drawer" app>
      ...
      <v-card class="d-flex justify-center pr-2 pl-2 mb-5" flat tile>
        <v-card-text>
          {{user.nama}}
          <br />
          <span class="caption">{{user.nim}}</span>
        </v-card-text>
      </v-card>
      <v-row>
        <p class="subtitle ml-8 mr-10">Selamat datang di aplikasi Beasiswa UIN Suska Riau</p>
      </v-row>
      <v-list dense>
        <!-- :to="page.to" -->
        ...
        <v-list-item router exact @click="routerPush('List LPJ')">
          ...
          <v-list-item-content>
            <v-list-item-title>LPJ</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list>
    </v-navigation-drawer>
      ...
      <div style="width:100%; -webkit-app-region: drag;">
        <v-toolbar-title>
          <router-link to="/mahasiswa/home">
            <span class="font-weight-bold ml-4 white--text">App Beasiswa</span>
          </router-link>
